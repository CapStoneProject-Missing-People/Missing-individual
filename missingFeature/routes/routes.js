...
} from "../controller/push-notification.controller.js";
import { requireAuth, isAdmin } from "../middleware/authMiddleware.js";
import {
  fetchAllMissingPeopleWithNames,
  updateImageBuffers,
...
import {
  GetMissingPerson,
  GetMergedId,
  UpdateMissingStatusToFound,
  UpdateMissingStatusToMissing,
  UpdateMissingStatusToPending,
} from "../controller/missingPersonController.js";
...
routers
  .route("/change-staus-found/:caseId/status")
  .put(requireAuth, UpdateMissingStatusToFound);

routers
  .route("/change-staus-missing/:caseId")
  .put(requireAuth, UpdateMissingStatusToMissing);

routers
  .route("/change-staus-pending/:caseId")
  .put(requireAuth, UpdateMissingStatusToPending);
...
routers
  ...
  .get(requireAuth, MarkNotificationAsRead);

routers.route("/guest-notifications").get(guestNotification);

// Admin Routes
routers
  .route("/get-images-with-names")
  .get(requireAuth, isAdmin([3244, 5150]), fetchAllMissingPeopleWithNames);

routers
  .route("/update-image/:missingId")
  .put(upload.any(), requireAuth, isAdmin([3244, 5150]), updateImageBuffers);
